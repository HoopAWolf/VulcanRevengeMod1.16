...
{
    private static final ResourceLocation SHULKER_SPARK_TEXTURE = new ResourceLocation(Reference.MOD_ID, "textures/entity/slothpet.png");
    private static final RenderType field_229123_e_ = RenderType.getEntityTranslucent(SHULKER_SPARK_TEXTURE);
    private final ShulkerBulletModel<SlothPetEntity> model = new ShulkerBulletModel<>();

    public SlothPetRenderer(EntityRendererManager manager)
    {
        super(manager);
    }
    ...
    @Override
    public void render(SlothPetEntity entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn)
    {
        matrixStackIn.push();
    ...
    @Override
    public ResourceLocation getEntityTexture(SlothPetEntity entity)
    {
