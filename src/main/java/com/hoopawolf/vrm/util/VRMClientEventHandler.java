        ...
        if (event.getEntityLiving().world.isRemote)
        {
            if (event.getItemStack().isEmpty())
            {
                if (event.getEntityLiving() instanceof PlayerEntity)
                {
                    ...
                    if (player.isCrouching() && player.inventory.armorInventory.get(3).getItem().equals(ArmorRegistryHandler.SLOTH_MASK_ARMOR.get()) && SinsArmorItem.isActivated(player.inventory.armorInventory.get(3)))
                    {
